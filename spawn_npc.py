...
def main():
    ...
    try:
        ...
        all_traffic_ligts = world.get_actors().filter('*.traffic_light')
        for tl in all_traffic_ligts:
            tl.set_yellow_time(5)
            tl.set_green_time(7)

        for s in world.get_actors().filter('*.stop'):
            s.destroy()
        ...
        def try_spawn_random_vehicle_at(transform):
            ...
            if can_spawn_vehicle(actor_list, transform):
        ...
        def can_spawn_vehicle(actor_list, transform):
